...
select distinct put(Date_Received, year4.) as Date_Received_Year
	from cs.claimsraw
	order by Date_Received_Year;

title "Years of Incidents";
select distinct put(Incident_Date, year4.) as Incident_Date_Year
	from cs.claimsraw
...
select Airport_Code, 
	...
	from claims_cleaned 
	group by Airport_Code, Airport_Name, Year;
select *
	from totalClaims
...
select count(*) as UnknownAirports
	from claims_cleaned
